__kernel void fitness(__global uchar* copyImage,__global uchar* originalImage, __global float* outputBuf )
{
	const int x = get_global_id(0);
	
	float value = fabs((float)originalImage[x] - (float)copyImage[x]);
